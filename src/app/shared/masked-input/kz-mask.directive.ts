...
export class KzMaskDirective implements ControlValueAccessor {
  ...
  onKeyup($event: any) {
    ...
    let pad:any
    if(valor.length <= 10){
      pad = this.kzMask2.replace(/\D/g, '').replace(/9/g, '_');
    }else{
      pad = this.kzMask.replace(/\D/g, '').replace(/9/g, '_');
    }
  ...
  onBlur($event: any) {
    if (($event.target.value.length === this.kzMask.length) || ($event.target.value.length === this.kzMask2.length)) {
      return;
    }
    this.onChange('');
    $event.target.value = '';
  ...
  aplicarMascara(valor: string): string {
    if(valor.length <= 10){
        return this.aplicMasc1(valor)

      }
      return this.aplicMasc2(valor)
  }

  aplicMasc1(valor: string): string{
    valor = valor.replace(/\D/g, '');
    let pad = this.kzMask2.replace(/\D/g, '').replace(/9/g, '_');
    let valorMask = valor + pad.substring(0, pad.length - valor.length);
    let valorMaskPos = 0;
    valor = '';
    for (let i = 0; i < this.kzMask2.length; i++) {
      if (isNaN(parseInt(this.kzMask2.charAt(i)))) {
        valor += this.kzMask2.charAt(i);
      } else {
  ...
  aplicMasc2(valor: string): string{
    valor = valor.replace(/\D/g, '');
